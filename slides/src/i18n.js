...
export const ZH = {
  ...
  'self-rendered': '自渲染',
  'Dart — wrong language': 'Dart —— 语言不对',
  'native UI': '原生 UI',
  'bound to React — wrong model': '绑定 React —— 编程模型不对',
  'plugs in — model half-survives': '接得上 —— 模型只保得住一半',
  'layout ≠ Web': '布局 ≠ Web',
  'no Web — low ROI': '不含 Web —— 投入产出比低',
...
export const ZH_NOTES = [
  ...
  // 34c Elk · 可拖拽 sheet · 三个归属者
  `<p><strong>可拖拽 sheet —— 难的不是"从底部滑上来",而是三套状态机协同。</strong>Vue Transition 管"sheet 是否存在于视觉状态"(开/关);一个主线程脚本 worklet 管"手指每一帧在哪"(拖动 + 橡皮筋);原生 <code>&lt;scroll-view&gt;</code> 管"这串触摸到底属于谁"(滚动 vs sheet)。三者边界划清,才同时拿到 Vue 的声明式状态、原生滚动与接近原生的跟手。</p><p><strong>结构即属性归属:</strong><code>sheet-surface-transition</code> 归 Vue Transition(CSS class),内层 <code>sheet-surface</code> 归拖动(inline <code>transform</code>)。若两者写同一元素的 transform,inline 覆盖 class —— 经典的"第一次能动,之后动画消失"。</p><p><strong>双线程红利:</strong>Web 上手势 JS、Vue 渲染、业务代码共享一条主线程,一有长任务就掉帧;这里拖拽跑在 UI 线程上,贴着原生输入与布局,Vue 在忙什么都不影响跟手。</p>`,
  // 34d Elk · 可拖拽 sheet · 用代码讲
