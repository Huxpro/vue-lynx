...
const a2uiPayloadPlugin: RsbuildPlugin = {
  ...
  setup(api) {
    api.onBeforeStartDevServer(({ server }) => {
      ...
      // Dev helper: report the rspeedy dev-server bundle URL for QR codes.
      // (Upstream registers this via `server.setup`, which our
      // @rsbuild/core version doesn't type.)
...
function buildRspeedyBundleUrl(port: number): string {
  ...
  return `http://${ip}:${port}/a2ui.lynx.bundle`;
}
