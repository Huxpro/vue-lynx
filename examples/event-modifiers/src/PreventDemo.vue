  ...
  <view :style="{ margin: '12px 12px 0', backgroundColor: '#fff', borderRadius: '10px', overflow: 'hidden' }">
    ...
    <view :style="{ padding: '12px 14px' }">
      <text :style="{ fontSize: '12px', color: '#444', lineHeight: '1.6' }">
        Lynx has no browser default actions to cancel — there is no {{ '<a>' }}
        navigation or {{ '<form>' }} submission. The .prevent modifier is
        accepted silently so that code written for the web can run on Lynx
        without modification, but it has no observable effect.
      </text>
