...
describe('structured template()', () => {
  ...
  it('preserves the dynamic text facade used by compiler-vapor $txt', () => {
    const factory = template(
      ['view', 0, [['text', 0, []]]] as never,
      1,
    );
    const root = factory();
    const textHost = root.firstChild!;
    expect(textHost.tag).toBe('text');
    expect(textHost.firstChild).not.toBeNull();
    expect(textHost.firstChild!.tag).toBe('#text');
    expect(textHost.firstChild!._text).toBe(' ');
  });
});
