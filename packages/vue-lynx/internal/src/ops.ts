...
/**
 * `PipelineOptions.dsl` reported by the Vapor renderer.
 *
 * Vapor and the vdom renderer share this ops protocol, so a single `dsl` would
 * make them indistinguishable in pipeline data — which is precisely the
 * comparison the data exists to support.
 */
export const DSL_VUE_VAPOR = 'vue-vapor';
