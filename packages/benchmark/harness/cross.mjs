...
// EXIST on native runtimes — we verified 50 update ticks on 10k rows going
// from >300 s (DNF) to ~15 s with the shim neutralized, with the worker
// otherwise idle. The patch below no-ops only `lynx.profile:`-prefixed
// entries and is applied identically to every framework (Vue never calls
// the profiling API, so it is unaffected either way).
