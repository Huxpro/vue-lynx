 ...
 * raw value OR a signal, takes a `DataContext`) into the simpler
 * `(args) => unknown` shape the renderer's `executeFunctionCall` expects.
 *
 * The Vue port's `resolveSignal` hands upstream impls Vue refs instead of
 * Preact signals, so results are unwrapped through both `isSignal` (upstream
 * signal instances created internally by `@a2ui/web_core`) and Vue's `isRef`.
 */
function adaptUpstreamImpl(impl: FunctionImplementation): FunctionImpl {
  return (args, context) => {
    ...
    if (isRef(result)) return result.value as unknown;
    return result;
  };
}
