...
import { useResolvedProps } from '../../vue/useDataBinding.js';
...
import '../../../../styles/a2ui/catalog/LineChart.css';
 ...
 */
export const LineChart: GenUIComponent = defineComponent({
  name: 'LineChart',
  props: catalogProps(
    'labels',
    'series',
  ...
  setup(rawProps: Record<string, unknown>) {
    const props = rawProps as unknown as LineChartProps;
    const [resolvedRef] = useResolvedProps(
      () => rawProps,
      () => props.surface,
    ...
    return (): VNodeChild => {
      const resolvedProps = resolvedRef.value;
      ...
      const series = Array.isArray(seriesValue)
        ? normalizeSeries(
          seriesValue as LineChartSeries[],
          DEFAULT_CHART_COLORS,
        )
        : [];
      ...
      return h('view', { class: 'line-chart' }, [
        h('view', { class: 'line-chart-header' }, [
          h('view', { class: 'line-chart-header-copy' }, [
            h(
              'text',
              { class: 'line-chart-title' },
              (resolvedProps['yLabel'] as string | undefined) ?? 'Line chart',
            ),
            h(
              'text',
              { class: 'line-chart-caption' },
              series.length > 0
        ...
        h('view', { class: 'line-chart-scale-row' }, [
          h('text', { class: 'line-chart-scale-value' }, formatValue(min)),
          h(
            'text',
            { class: 'line-chart-scale-value line-chart-scale-value-right' },
            formatValue(max),
          ),
        ]),
        h('svg', {
        ...
        h(
          'view',
          { class: 'line-chart-axis-row' },
          visibleLabelIndices.map((index) =>
            h(
              'view',
              {
                key: `${index}-${labels[index] ?? index}`,
                class: 'line-chart-axis-tick',
              },
              [
                h(
                  'text',
                  { class: 'line-chart-axis-tick-label' },
                  labels[index] ?? '',
                ),
              ],
            )
        ...
        showLegend
          ? h(
            ...
            series.map((item, index) =>
              h(
                'view',
                { key: item.name, class: 'line-chart-legend-item' },
                [
                  h('view', {
