/**
 * Build the four-cell matrix (vdom/vapor × IFR off/on) from the SAME
 * generated SFC and collect the .web.bundle files where
 * web-harness/run-browser.mjs expects them:
 *
 ...
 * the MT increment is route c's core selling point and must be reported.
 */
...
const CELLS = [
  { name: 'content-vdom', mode: 'vdom', ifr: '0' },
  { name: 'content-vdom-ifr', mode: 'vdom', ifr: '1' },
  { name: 'content-vapor', mode: 'vapor', ifr: '0' },
  { name: 'content-vapor-ifr', mode: 'vapor', ifr: '1' },
];
...
for (const cell of CELLS) {
  ...
  fs.rmSync(path.join(_dirname, 'node_modules/.cache'), { recursive: true, force: true });
  fs.rmSync(path.join(_dirname, 'dist'), { recursive: true, force: true });
  console.log(`[build] ${cell.name} (vapor=${cell.mode === 'vapor' ? 1 : 0} ifr=${cell.ifr})`);
  execFileSync(
  ...
  const lynxRaw = fs.existsSync(lynxBundlePath) ? fs.readFileSync(lynxBundlePath) : null;
  sizes.push({
    cell: cell.name,
...
for (const s of sizes) {
  console.log(
    `${s.cell.padEnd(20)} web ${String(s.webBundle.raw).padStart(8)}B (gz ${s.webBundle.gzip})  MT ${
