...
- Wired as an entry in ops-apply's `STAGING_STRATEGIES` table with its flag read in `main-thread/src/flags.ts` (`codePaintRequested()`), so retiring the axis later is: delete the strategy entry, `code-template.ts`, and the flag accessor. Bundle-delivered structures (`+b!`, REGISTER_TREE_BUNDLE) feed the same strategy hooks.
