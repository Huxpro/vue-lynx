...
为了优化滚动条的性能，我们需要引入[主线程脚本（MTS）](/zh/guide/main-thread-script)来[在主线程上处理事件](https://lynxjs.org/next/guide/interaction/event-handling.html#main-thread-event-processing)，将我们在上一步中对滚动条高度和位置的调整从后台线程迁移到主线程。
...
- `:main-thread-bindscroll="ctx"` — 将 MTS 处理器绑定到滚动事件（在主线程上运行，零线程切换）
