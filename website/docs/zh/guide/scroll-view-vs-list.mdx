...
### 3. Composable 管分页；`<list>` 管视口

在 Web 上，无限滚动通常是：

`useInfiniteQuery` / 自定义 composable **+** 虚拟列表 **+** IntersectionObserver 哨兵。

在 Lynx 里可以去掉虚拟列表。Composable 往 `ref` 数组追加数据；`@scrolltolower`（配合 `lower-threshold-item-count`）请求下一页。原生 `<list>` 只会把**可见** cell 挂到视口——这是平台能力，不等于 Vue Lynx 已经做完框架侧 cell 回收（见 [#302](https://github.com/Huxpro/vue-lynx/issues/302)）。在主线程真正 defer / 卸载 cell 之前，超长 feed 请用有界窗口或分页策略，不要假设无界 `ref` 数组内存恒定。
...
:::tip List diff
Vue Lynx 主线程 list 适配层通过对比「上次 flush 快照」与当前 `listItems`（LIS 检测 move，语义对齐 ReactLynx 的 remove+insert）刷新 `insertAction` / `removeAction` / `updateAction`。覆盖追加、头部插入、中间插入、删除、同 list 重排、platform-info 更新、框架侧 cell 回收（`enqueueComponent` / recycle pool — [#302](https://github.com/Huxpro/vue-lynx/issues/302)），以及每次 list flush 刷新 `__UpdateListCallbacks`（[#303](https://github.com/Huxpro/vue-lynx/issues/303)）。
:::
...
| `ListRecycle` | leave→re-enter uiSign 复用 | 已修 — `enqueueComponent` 池 (#302) |
...
点 **Run recycle probe**。主线程 worklet 通过 `componentAtIndex` / `enqueueComponent` leave→re-enter，必须显示 **self-reuse: PASS** 与 **cross-item: PASS**（池中同一 uiSign）。

:::note 内存
当前池只是把 uiSign 所有权重映射给原生回收协议。Vue 仍会 eager CREATE 每个 `list-item` 子树，因此主线程树数量仍是 **O(数据条数)**，直到创建真正 defer。跨 item hydrate 需要显式共享的 `reuse-identifier`，且仅适用于**同构（uniform）子树**；空 identifier 只做 self-reuse。内部 `v-if` / `v-for` 导致形状不同的 cell 会跳过跨 item hydrate，而不会做结构 morph。
:::
...
自动化：`packages/testing-library` → **native list element · mutations** + **list cell recycling** + **list UpdateListCallbacks**。
