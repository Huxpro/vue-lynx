...
:::note 内存
当前池只是把 uiSign 所有权重映射给原生回收协议。Vue 仍会 eager CREATE 每个 `list-item` 子树，因此主线程树数量仍是 **O(数据条数)**，直到创建真正 defer。跨 item hydrate 需要显式共享的 `reuse-identifier` 且子树同构；空 identifier 只做 self-reuse。
:::
