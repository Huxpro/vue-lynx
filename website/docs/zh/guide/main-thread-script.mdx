...
作为逐函数标注指令的实验性替代方案，你可以把组件的所有主线程代码集中到一个
专用的 `<script main>` SFC 代码块中——其中每个顶层函数都会成为主线程函数，
编译进本页描述的完全相同的 worklet 机制。
...
完整语法、语义与迁移指南见 [`<script main>` 代码块](/zh/guide/script-main)。
