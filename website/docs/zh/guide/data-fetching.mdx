...
Lynx 提供了 [Fetch API](https://lynxjs.org/next/api/lynx-api/global/fetch) 用于发起网络请求。虽然你可以直接使用 `fetch()`，但我们推荐使用 [TanStack Query (Vue Query)](https://tanstack.com/query/v5/docs/framework/vue/overview) 来管理服务端状态——它开箱即用地处理了缓存、后台重新获取、过期数据和数据变更等问题。
