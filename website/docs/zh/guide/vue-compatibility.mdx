...
:::warning Lynx 引擎兼容性
显式 `<page>` 根节点复用已在 iOS 18.5 和 iOS 26.2 模拟器的 Lynx SDK 1.4.0 上验证。更早的 Lynx SDK 尚未验证。依赖显式 `<page>` 包装时，请使用 SDK 1.4.0 或更高版本；如果需要兼容更早的引擎，请省略该包装，让 Vue Lynx 使用自动创建的原生 page。
:::
