...
Vue 的[事件修饰符](https://vuejs.org/guide/essentials/event-handling.html#event-modifiers)（`.once`、`.stop`、`.prevent`、`.self`）在 Lynx 上可用，但有一些平台特有的差异需要注意。
...
| `.prevent` | Lynx 上无效果 | 会正确调用 `preventDefault()`，但 Lynx 没有浏览器默认行为可取消（没有 `<a>` 导航、没有 `<form>` 提交）。为 API 完整性而保留。 |
