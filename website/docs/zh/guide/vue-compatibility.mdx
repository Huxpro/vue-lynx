...
一个页面只能有一个 `<page>` 包装，并且它必须是表示整个页面的最外层元素。不要嵌套或同时渲染多个 `<page>`。与 Lynx 本身一致，page 的尺寸由外层原生 View 决定，不能通过 `width`、`height`、`left` 或 `top` 直接改变 page 的大小或位置。

:::warning Lynx 引擎兼容性
显式 `<page>` 根节点复用已在源码构建的 Explorer 中，以 iOS 18.5 模拟器上的 Lynx Engine 4.1（[`SystemInfo.engineVersion`](https://lynxjs.org/api/lynx-api/global/system-info#engineversion)）完成验证。更早的引擎版本尚未验证。如果需要兼容尚未验证的旧引擎，请省略该包装，让 Vue Lynx 使用自动创建的原生 page。
