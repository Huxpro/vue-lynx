...
:::info Lynx 特有限制
如果你知道 CSS 时长，请优先传递显式的 `:duration` prop——后台线程无法使用 `getComputedStyle()`。不传时，Vue Lynx 会等待真实的 `transitionend`/`animationend` 事件，并附带有界的回退清理，避免被打断的过渡永久泄漏 event-registry 处理器（[#286](https://github.com/Huxpro/vue-lynx/issues/286)）。`<TransitionGroup>` 支持进入和离开过渡，但不支持移动（FLIP）动画，因为 `getBoundingClientRect()` 不可用。
:::

在下面的示例中滚动到 **9. Registry cleanup**：快速 hammer 一个没有 `:duration` 的 `<Transition>`，观察 event-registry 计数上升；停止后，修复后的版本会在约 4 秒内回到 baseline，而不会永久泄漏。
...
| 特性 | 原因 | 替代方案 |
|---------|--------|-------------|
| `<Transition>` 基于 CSS 的自动时长 | 后台线程无法使用 `getComputedStyle()` | 优先传显式 `:duration`；不传时仍可通过事件 + 有界回退清理正常工作（[#286](https://github.com/Huxpro/vue-lynx/issues/286)） |
