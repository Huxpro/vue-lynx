...
查看详细的[快速开始](/guide/quick-start)文档，它将指导你创建一个新的 Lynx 项目。我们推荐使用 TypeScript，以获得更好的开发体验，包括静态类型检查和更好的编辑器智能提示。
...
为了让点赞有更好的视觉交互效果，我们在 `gallery.css` 中添加了 CSS 动画。你也可以在[动画](https://lynxjs.org/next/guide/styling/animation)章节了解更多，并替换为你喜欢的样式！
...
类似于用于添加点赞功能的 `@tap` 事件，我们给 `<list>` 添加 [`scroll`](https://lynxjs.org/next/api/elements/built-in/list.html#scroll) 事件，它会在 `<list>` 元素滚动时触发。为了让滚动条更快地响应滚动事件，我们需要将 [`scroll-event-throttle`](https://lynxjs.org/next/api/elements/built-in/list.html#scroll-event-throttle) 设为 0。
...
为了优化滚动条的性能，我们需要引入[主线程脚本 (MTS)](https://lynxjs.org/next/react/main-thread-script.html) 来[在主线程上处理事件](https://lynxjs.org/next/guide/interaction/event-handling.html#main-thread-event-processing)，将上一步中对滚动条高度和位置的调整从背景线程迁移到主线程。
