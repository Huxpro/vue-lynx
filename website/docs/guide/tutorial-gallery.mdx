...
Unlike React Lynx which uses `root.render(<App />)`, Vue Lynx uses `createApp(Component).mount()`. This creates the Vue application and mounts it to the Lynx page root.
:::
...
:::details What is "@tap"?
In Vue Lynx, event binding uses `@eventName` syntax (short for `v-on:eventName`). This is equivalent to React Lynx's `bindtap={handler}`. Lynx follows the `bind*` convention for events due to the static nature of its architecture. Learn more on the [Event Handling](https://lynxjs.org/next/guide/interaction/event-handling) page.
:::
