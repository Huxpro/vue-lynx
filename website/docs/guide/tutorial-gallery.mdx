...
> [!NOTE]
> The finished `GalleryComplete` source keeps its two main-thread functions in
> an experimental [`<script main>` block](/guide/script-main) — same worklets,
> no per-function `'main thread'` directives.
