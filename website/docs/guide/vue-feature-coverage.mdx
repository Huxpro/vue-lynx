...
1. **Async `setup()`** — top-level `await` in `<script setup>`, with the `#fallback` slot shown until the promise resolves
2. **Lazy-loaded counter** — `defineAsyncComponent` with dynamic `import()` for code-splitting
