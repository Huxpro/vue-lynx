...
> [!NOTE]
> The `SwiperMTS` source in the viewer groups its three touch handlers in an
> experimental [`<script main>` block](/guide/script-main) instead of
> repeating the `'main thread'` directive in each one — the semantics are
> identical; the block compiles into exactly the worklets described above.
