...
The `vue-lynx-testing-library` package offers APIs like `render`, `fireEvent`, and `screen` for testing Vue Lynx components, similar to [Vue Test Utils](https://test-utils.vuejs.org/) and [React Testing Library](https://testing-library.com/), with the dual-threaded architecture abstracted through [`@lynx-js/testing-environment`](https://www.npmjs.com/package/@lynx-js/testing-environment).
...
import { render, fireEvent, screen } from 'vue-lynx-testing-library';
...
it('basic', async () => {
  ...
  const { container } = render(
    defineComponent({
      setup() {
        return () =>
          h(Button, { onClick }, [
            h('text', { 'data-testid': 'text' }, 'Click me'),
          ]);
      },
    }),
  );
  ...
  // ACT
  fireEvent.tap(container.firstElementChild!);
...
VueLynx Testing Library uses JSDOM to implement [Element PAPI](https://lynxjs.org/guide/spec.html), ensuring compatibility with [`@testing-library/jest-dom`](https://github.com/testing-library/jest-dom) matchers like `toHaveTextContent`.
...
it('basic render', () => {
  const Comp = defineComponent({
    render() {
      return h('view', { 'data-testid': 'inner', style: { backgroundColor: 'yellow' } });
    },
  });
...
it('fireEvent', async () => {
  ...
  const Comp = defineComponent({
    setup() {
      return () => h('text', { catchtap: handler });
    },
  });
...
Vue Lynx sets a `vue-ref-{id}` attribute on elements, which can be used for snapshot testing to verify ref assignment. The `ShadowElement` (background thread representation) provides `NodesRef` methods like `invoke`, `setNativeProps`, and `animate`:
...
The `screen` object provides query methods like `getByText` and `getByTestId`:

```ts
import '@testing-library/jest-dom';
import { h, defineComponent, ref, nextTick } from 'vue-lynx';
import { render, screen, waitForUpdate } from 'vue-lynx-testing-library';
import { expect, it } from 'vitest';
...
it('queries rendered elements', async () => {
  ...
  const Comp = defineComponent({
    setup() {
      return () =>
        loaded.value
          ? h('text', { 'data-testid': 'message' }, 'Hello World')
          : h('text', null, 'Loading...');
    },
  });
...
it('rerender will re-render your component', () => {
  ...
  expect(container.querySelector('text')!).toHaveTextContent('hi');

  {
    const { container } = rerender(Greeting, { message: 'hey' });
    expect(container.querySelector('text')!).toHaveTextContent('hey');
  }
});
```
...
For additional examples, see the test cases in the [vue-lynx testing-library source code](https://github.com/nicepkg/vue-lynx/tree/main/testing-library/src/__tests__).
...
| `getByText()`, `getByTestId()`, ... | All [`@testing-library/dom`](https://testing-library.com/docs/queries/about) queries, bound to the container |
...
A convenience object from `@testing-library/dom` for querying the rendered output:
