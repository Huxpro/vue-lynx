...
Lynx does not provide browser globals. Libraries that depend on `document` or `window` need adaptation to work. Due to the dual-thread architecture, Lynx provides its own [APIs](https://lynxjs.org/next/api/lynx-api/lynx-api-overview.html) that account for which thread your code runs on:

**Global APIs** — available on both threads, same as the web:
...
| `getBoundingClientRect()` | `NodesRef.invoke({ method: 'boundingClientRect' })` | Background (async) |
| `getComputedStyle()` | [`element.getComputedStyleProperty()`](https://lynxjs.org/next/api/lynx-api/main-thread/element-get-computed-style.html) | Main thread (sync, MTS) |
| `addEventListener('resize')` | `main-thread-bindlayoutchange` | Main thread (sync, MTS) |
...
When you need synchronous access — smooth animations, gesture handling, layout measurement — use [Main Thread Script](/guide/main-thread-script). Functions marked `'main thread'` run directly on the main thread and can call [`getComputedStyleProperty()`](https://lynxjs.org/next/api/lynx-api/main-thread/element-get-computed-style.html), listen to `main-thread-bindlayoutchange`, and access native elements via `useMainThreadRef()`:
