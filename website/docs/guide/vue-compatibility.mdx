...
## Transition <Badge text="v0.5" type="tip" />

Vue's `<Transition>` and `<TransitionGroup>` components apply enter/leave animations when elements are inserted or removed.

Since [#248](https://github.com/Huxpro/vue-lynx/issues/248) (v0.5), Vue Lynx also follows Vue's persisted transition behavior for `v-show`: enter and leave hooks run on every visibility change, and `display: none` is applied only after the leave animation finishes. This makes patterns such as drawers, sheets, popovers, and other state-preserving overlays behave consistently with official Vue on both Web and native Lynx.
