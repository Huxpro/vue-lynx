...
Requires **Lynx engine ≥ 3.9.0**, the first release whose tagged source contains the fix that propagates CSS custom properties from the component root to its descendants ([lynx-family/lynx#5912](https://github.com/lynx-family/lynx/pull/5912)). On earlier engines the variables are stamped on the root but never reach descendants.
