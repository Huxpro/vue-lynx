...
Only one `<page>` wrapper is allowed, and it must be the outermost element representing the screen. Do not nest it or render multiple wrappers. As with Lynx itself, the containing native view controls the page dimensions: `width`, `height`, `left`, and `top` cannot resize or reposition the page directly.

:::warning Lynx engine compatibility
Explicit `<page>` root reuse has been verified with Lynx Engine 4.1 ([`SystemInfo.engineVersion`](https://lynxjs.org/api/lynx-api/global/system-info#engineversion)) in a source-built Explorer on an iOS 18.5 simulator. Earlier engine versions have not been verified. When targeting an unverified older engine, omit the wrapper and let Vue Lynx use the automatically created native page.
...
:::info Main-thread (worklet) event handlers
