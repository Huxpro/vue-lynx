...
Vue SFCs support several CSS features in `<style>` blocks. Not all of them work on Lynx today due to CSS engine constraints.

| Feature | Status | Notes |
|---------|--------|-------|
| `<style>` (plain) | **Works** | Class selectors applied via CSS extraction |
| Imported `.css` files | **Works** | Same extraction pipeline |
| `<style module>` | **Works** | Hashed class names are plain class selectors |
| `<style scoped>` | Not yet supported | See [#78](https://github.com/user/vue-lynx/issues/78) |
| `v-bind()` in `<style>` | Not yet supported | See [#79](https://github.com/user/vue-lynx/issues/79) |

**Plain `<style>`** blocks and **imported `.css` files** work out of the box — CSS is extracted by `@lynx-js/css-extract-webpack-plugin` and applied via class selectors.

**`<style module>`** ([CSS Modules](https://vuejs.org/api/sfc-css-features.html#css-modules)) works because `css-loader` transforms class names to unique hashed names (e.g. `.card` → `.card-k33EaR`), which are still plain class selectors that Lynx supports. Use `:class="$style.card"` to apply them.

:::warning Not yet supported
**`<style scoped>`** does not work because Vue's scoped CSS compiles to `[data-v-xxx]` attribute selectors, which Lynx's CSS engine does not support. Tracked in [#78](https://github.com/Huxpro/vue-lynx/issues/78).

**`v-bind()` in `<style>`** causes a build error because the compiled code requires `useCssVars` from `@vue/runtime-dom`, which is not available in vue-lynx's `@vue/runtime-core`-based runtime. Tracked in [#79](https://github.com/Huxpro/vue-lynx/issues/79).

For **dynamic styling**, use computed `:style` bindings as a workaround:
```vue
<script setup>
...
The example below demonstrates each pattern and its current status:
