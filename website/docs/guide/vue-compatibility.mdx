...
| `<style module>` | Works | Hashed class names (e.g. `.card-k33EaR`) are plain class selectors |
| `<style scoped>` | Not yet supported ([#78](https://github.com/Huxpro/vue-lynx/issues/78)) | Compiles to `[data-v-xxx]` attribute selectors; Lynx CSS engine only supports class selectors |
| `v-bind()` in `<style>` | Not yet supported ([#79](https://github.com/Huxpro/vue-lynx/issues/79)) | Requires `useCssVars` from `@vue/runtime-dom`, unavailable in vue-lynx |
