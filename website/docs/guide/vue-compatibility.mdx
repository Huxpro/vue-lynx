...
Vue's [event modifiers](https://vuejs.org/guide/essentials/event-handling.html#event-modifiers) (`.once`, `.stop`, `.prevent`, `.self`) work on Lynx, with a few platform-specific differences to be aware of.
...
| `.prevent` | No-op on Lynx | Calls `preventDefault()` correctly, but Lynx has no browser default actions to cancel (no `<a>` navigation, no `<form>` submission). Included for API completeness. |

:::

The example below demonstrates all four modifiers side by side:
