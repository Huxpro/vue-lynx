...
Explicit `<page>` root reuse is verified with Lynx SDK 1.4.0 on iOS 18.5 and iOS 26.2 simulators. Older Lynx SDKs have not been verified. Use SDK 1.4.0 or later when relying on an explicit `<page>` wrapper; when targeting an older engine, omit the wrapper and let Vue Lynx use the automatically created native page.
:::
