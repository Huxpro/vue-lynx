...
By default, main thread functions cannot directly call plain functions that do not have the `'main thread'` directive. This makes code reuse difficult. The `shared-module` mechanism allows you to explicitly declare certain modules as shareable between the main thread and the background thread.

<Go
  example="main-thread"
  defaultEntryFile="dist/shared-module.lynx.bundle"
  defaultFile="src/shared-module/App.vue"
  entry={["src/shared-module", "src/shared-module/color-utils.ts"]}
/>
