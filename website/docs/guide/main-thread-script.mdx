...
Sometimes you have utility functions — pure computation, color helpers, math — that need to run on **both** threads. Normally, modules imported in a main thread function are only available on the main thread, and regular imports are only available on the background thread.

The `with { runtime: 'shared' }` import attribute tells Vue Lynx to include a module's code in both thread bundles:
