...
As an experimental alternative to per-function directives, you can group all
of a component's main-thread code into a dedicated `<script main>` SFC block —
every top-level function in it becomes a main thread function, compiled into
the exact same worklet machinery described on this page.
...
See [The `<script main>` Block](/guide/script-main) for the full syntax,
semantics, and migration guide.
