...
:::note Memory
The current pool remaps uiSign ownership for the native recycler protocol. Vue still eagerly CREATEs every `list-item` subtree, so total main-thread trees remain **O(data items)** until creation is deferred. Cross-item hydrate needs an explicit shared `reuse-identifier` and isomorphic shapes; empty identifiers only self-reuse.
:::
