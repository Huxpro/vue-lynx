...
### 3. Composable owns pages; `<list>` owns the viewport

On the Web, infinite feeds usually mean:

`useInfiniteQuery` / custom composable **+** a virtualizer **+** an intersection sentinel.

On Lynx, drop the virtualizer. A composable appends to a `ref` array; `@scrolltolower` (and `lower-threshold-item-count`) asks for the next page. Native `<list>` **lazy-attaches** only visible cells to the viewport — that is a platform capability, not yet full framework-side cell recycling in Vue Lynx (see [#302](https://github.com/Huxpro/vue-lynx/issues/302)). Until cells are deferred/unrendered on the main thread, prefer a bounded window or pagination strategy for very long feeds rather than an unbounded `ref` of every row.
...
:::tip List diffs
Vue Lynx's Main Thread list adapter flushes `insertAction` / `removeAction` / `updateAction` by diffing the last-flushed snapshot against the live `listItems` array (LIS move detection, same idea as ReactLynx's remove+insert moves). Covers append, prepend, mid-list insert, remove, same-list reorder, platform-info updates, framework-side cell recycling (`enqueueComponent` / recycle pool — [#302](https://github.com/Huxpro/vue-lynx/issues/302)), and refreshing `__UpdateListCallbacks` on every list flush ([#303](https://github.com/Huxpro/vue-lynx/issues/303)).
:::
...
| `ListRecycle` | leave→re-enter uiSign reuse | Fixed — `enqueueComponent` pool (#302) |
...
Tap **Run recycle probe**. A main-thread worklet leave→re-enters cells via `componentAtIndex` / `enqueueComponent` and must report **self-reuse: PASS** and **cross-item: PASS** (same uiSign from the pool).

:::note Memory
The current pool remaps uiSign ownership for the native recycler protocol. Vue still eagerly CREATEs every `list-item` subtree, so total main-thread trees remain **O(data items)** until creation is deferred. Cross-item hydrate needs an explicit shared `reuse-identifier` and **isomorphic (uniform) shapes**; empty identifiers only self-reuse. Cells that differ via inner `v-if` / `v-for` skip cross-item hydrate rather than morph trees.
:::

<Go
  example="scrolling"
  defaultFile="src/ListRecycle/App.vue"
  entry="src/ListRecycle"
  defaultEntryName="ListRecycle"
/>

Automated coverage: `packages/testing-library` → **native list element · mutations** + **list cell recycling** + **list UpdateListCallbacks**.
