...
Lynx provides the [Fetch API](https://lynxjs.org/next/api/lynx-api/global/fetch) for making network requests. While you can use `fetch()` directly, we recommend [TanStack Query (Vue Query)](https://tanstack.com/query/v5/docs/framework/vue/overview) for managing server state — it handles caching, background refetching, stale data, and mutations out of the box.
